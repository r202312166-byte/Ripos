...
#endif
{
    ...
    config.isolated = 0;
    config.safe_path = 0;
    // allow PYTHONPATH so Lib/ can be found during the build
...
#define CLEAR(ATTR) \
    ...
    // Deterministic hash seed: no OS randomness source is guaranteed on
    // this platform (host builds on mingw).  Host tooling only.
