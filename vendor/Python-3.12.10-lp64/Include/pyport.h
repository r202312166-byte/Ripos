...
#    define HAVE_THREAD_LOCAL 1
     /* Freestanding kernel build: the two thread-local variables
        (_Py_tss_tstate in pystate.c, pkgcontext in import.c) become plain
        globals.  On a single-CPU cooperative kernel the GIL serializes
        access, so a global thread-state is correct, and it removes the need
        for %fs-based TLS emulation in the kernel. */
