...
fn start(disk_number: u16, partition_table_start: *const u8) -> ! {
    ...
    // Ripos fork: stock 1280x720 resolves to 800x600 on VirtualBox (the
    // VESA list jumps 800x600 -> 1024x768, whose height exceeds 720).
    // Raised so the OS gets a native 1280x1024 / 1600x1200 class screen.
    let max_width = 1920;
    let max_height = 1080;
